...
lengthSlider.addEventListener('input', (evente) => {
    passwordLength = evente.target.value;
    handleSlider();
});
...
function generateNumber() {
    // generate from 1-9 here 10 is exclusive
...
// calculate password strength & set Indicator
function setIndicator(color) {
    indicator.style.backgroundColor = color;
    indicator.style.boxShadow = `0px 0px 5px 1.5px ${color}`;
}
...
function shuffleArray(array) {
    for (let i = array.length - 1; i > 0; i--) {
        // Find a random index j within the current range [0, i]
